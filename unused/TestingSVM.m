...
% TRAIN AND TEST DATA
SVMModels = trainSVM(TrainingSet(:, 1:end-1), TrainingSet(:, end), 'RBF');

trainAcc = testSVM(TrainingSet(:, 1:end-1), TrainingSet(:, end), SVMModels);
testAcc = testSVM(TestSet(:, 1:end-1), TestSet(:, end), SVMModels);
fprintf('TrainAcc = %d, TestAcc = %d', trainAcc. testAcc);