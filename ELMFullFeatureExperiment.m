...
for iFile=1:length(fileNames)
    ...
    for iClass=1:length(classNum)
        ...
        for i=1:nClasses
            ...
            trainingData = ...
                [trainingData; features(ithClassInd(1:nithClass), :)];
            testingData = ...
        ...
        for iExp=1:MAX_EXPERIMENT
            ...
            fprintf('Building iFile = %d/%d, iClass = %d/%d, iExp = %d/%d\n', ...
                iFile, length(fileNames), iClass, ...
                length(classNum), iExp, MAX_EXPERIMENT);
            oneExperiment = zeros(MAX_ITERATION, 3);
            for iIter=1:MAX_ITERATION
                nHiddenNode = ceil((maxTotalHiddenNodes - minTotalHiddenNodes) ...
                    .* rand() + minTotalHiddenNodes);

                % TRAINING
                % prepare the target data
                % (example: transformation from 4 into [0 0 0 1 0 0])
                trainingTarget = full(ind2vec(trainingData(:,end)'))'; 
                [Model, trainAcc] = trainELM(trainingData(:,1:end-1), ...
                    trainingTarget, nHiddenNode);
            ...
            bestOfOneExperiment = oneExperiment(oneExperiment(:, 2) == ...
                max(oneExperiment(:, 2)), :);
            if length(bestOfOneExperiment) > 1
                bestOfOneExperiment = ...
                    bestOfOneExperiment(bestOfOneExperiment(:, 1) == ...
                    max(bestOfOneExperiment(:, 1)), :);
                if length(bestOfOneExperiment) > 1
                    bestOfOneExperiment = ...
                        bestOfOneExperiment(bestOfOneExperiment(:, 3) ...
                        == min(bestOfOneExperiment(:, 3)), :);
                end
            end
            bestOfOneExperiment = bestOfOneExperiment(1, :);
            xlswrite(sprintf('%s/ELM_%s_experiment_%d.xlsx', eachFileFolder, ...
                fileNames{iFile}, iExp), ...
                {'TrainAcc', 'TestAcc', 'HiddenNode'}, ...
                sprintf('%d classes', nClasses));
            xlswrite(sprintf('%s/ELM_%s_experiment_%d.xlsx', eachFileFolder, ...
                fileNames{iFile}, iExp), oneExperiment, ...
                sprintf('%d classes', nClasses), 'A2')
            timeNeeded = toc;
            allExperiments(iExp, :) = [bestOfOneExperiment timeNeeded];
        end
        
        xlswrite(sprintf('%s/ELM_%s_experiment_result.xlsx', ...
            eachFileFolder, fileNames{iFile}), ...
            {'TrainAcc', 'TestAcc', 'HiddenNode', 'Time (sec)'}, ...
            sprintf('%d classes', nClasses));
        xlswrite(sprintf('%s/ELM_%s_experiment_result.xlsx', ...
            eachFileFolder, fileNames{iFile}), allExperiments, ...
            sprintf('%d classes', nClasses), 'A2')
        bestOfAllExperiments = allExperiments(allExperiments(:, 2) == ...
            max(allExperiments(:, 2)), :);
        if length(bestOfAllExperiments) > 1
            bestOfAllExperiments = ...
                bestOfAllExperiments(bestOfAllExperiments(:, 1) == ...
                max(bestOfAllExperiments(:, 1)), :);
            if length(bestOfAllExperiments) > 1
                bestOfAllExperiments = ...
                    bestOfAllExperiments(bestOfAllExperiments(:, 3) == ...
                    min(bestOfAllExperiments(:, 3)), :);
            end
        end
        bestOfAllExperiments = bestOfAllExperiments(1, :);
        xlswrite(sprintf('%s/ELM_result.xlsx', rootFolder), ...
            {'File Name', 'TrainAcc', 'TestAcc', 'HiddenNode', 'Time (sec)'}, ...
            sprintf('%d classes', nClasses));
        xlswrite(sprintf('%s/ELM_result.xlsx', ...
            rootFolder), fileNames(iFile), ...
            sprintf('%d classes', nClasses), sprintf('A%d', iFile+1))
        xlswrite(sprintf('%s/ELM_result.xlsx', ...
            rootFolder), bestOfAllExperiments, ...
            sprintf('%d classes', nClasses), sprintf('B%d', iFile+1))
    end
end